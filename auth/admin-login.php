...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$email || !$pw) {
        ...
        $st = db()->prepare('SELECT id, name, password, role FROM admins WHERE email = :e LIMIT 1');
        $st->execute([':e' => $email]);
        $u = $st->fetch();
...
<title>Admin Login — FleetSimplify</title>
...
<div class="auth-card">
    ...
    <h1>Admin Sign In</h1>
    <p class="auth-sub">Operations console.</p>
