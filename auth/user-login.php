...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$email || !$pw) {
        ...
        $st = db()->prepare('SELECT id, name, password FROM users WHERE email = :e LIMIT 1');
        $st->execute([':e' => $email]);
        $u = $st->fetch();
...
<title>Driver Login — FleetSimplify</title>
...
<div class="auth-card">
    ...
    <form method="post" data-validate-form novalidate>
        ...
        <div class="form-row">
            <label>Email</label>
            <input type="email" name="email" data-validate="email" value="<?= e($email) ?>" autocomplete="email">
            <span class="field-error"></span>
        </div>
        <div class="form-row">
            <label>Password</label>
            <div class="password-wrap">
                <input type="password" name="password" data-validate="required" autocomplete="current-password">
    ...
    <div class="auth-foot">No account? <a href="<?= e(url('auth/user-register.php')) ?>">Register</a></div>
